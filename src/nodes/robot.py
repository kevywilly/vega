...
class Robot(Node):
    ...
    def process_joy(self, data: Dict):
        ...
        if dir == 'N':
            self.gait = Trot(p0=POSITIONS.READY, stride=50, clearance=50, step_size=10)
        elif dir == "S":
            self.gait = Trot(p0=POSITIONS.READY, stride=-50, clearance=50, step_size=10)
        elif dir == "E" and jid == 1:
            self.gait = Sidestep(p0=POSITIONS.READY, stride=25, clearance=40, step_size=10)
        elif dir == "E" and jid == 2:
            self.gait = Turn(degrees=-20, p0=POSITIONS.READY, clearance=50, step_size=10)
